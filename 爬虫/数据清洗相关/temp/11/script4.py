...
def TEst():
    ...
    f = open("29/json/product_ids_text3.json", 'r')
...
if __name__ == '__main__':
    ...
    # 读取第一个 JSON 文件
    with open("product_ids_text1.json", 'r') as f:
        goods_ids1 = json.load(f)
        for goods_id1 in goods_ids1:
            product_ids.add(goods_id1)

    # 读取第二个 JSON 文件
    with open("product_ids_text2.json", 'r') as f:
        goods_ids2 = json.load(f)
        for goods_id1 in goods_ids2:
            product_ids.add(goods_id1)

    # 将 set 转换为 list，然后写入到新的 JSON 文件
    with open("product_ids.json", 'w') as json_file:
        json.dump(list(product_ids), json_file, ensure_ascii=False)
